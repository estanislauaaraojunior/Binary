...
def get_signal(prices: list, adx_min: float = ADX_MIN) -> SignalResult:
    ...
    ema50     = ind.ema(prices, EMA_TREND) if USE_EMA_TREND_FILTER else None
    ...
    atr_val   = ind.atr(prices, ATR_PERIOD)
    ...
    last_price = prices[-1]
    atr_norm = (atr_val / last_price) if (atr_val is not None and last_price != 0) else 0.0
    ...
    if not (RSI_OVERSOLD <= rsi_val <= RSI_OVERBOUGHT):
        return None, indicators
    # ── Filtro 3: EMA50 — tendência de médio prazo (USE_EMA_TREND_FILTER) ──
    # EMA9 e EMA21 devem estar no mesmo lado da EMA50 para confirmar tendência.
    if USE_EMA_TREND_FILTER and ema50 is not None:
        indicators["ema50"] = round(ema50, 5)
        trending_up   = ema9 > ema50 and ema21 > ema50
        trending_down = ema9 < ema50 and ema21 < ema50
        if not (trending_up or trending_down):
            return None, indicators
