...
def _compute_feature_map(prices: list) -> Optional[dict]:
    ...
    # ATR normalizado
    atr_v    = ind.atr(prices, ATR_PERIOD)
    atr_norm = (atr_v / price) if (atr_v is not None and price != 0) else 0.0

    # Stochastic %K e %D
    stoch_res = ind.stochastic(prices, STOCH_K_PERIOD, STOCH_D_PERIOD)
    ...
    ema50       = ind.ema(prices, EMA_TREND)
    ema50_cross = (ema9 - ema50) / price if (ema50 is not None and price != 0) else 0.0

    # Variação da largura das Bollinger Bands
    bb_width_change = 0.0
    if bb_ is not None and len(prices) > BB_PERIOD:
        bb_prev = ind.bollinger(prices[:-1], BB_PERIOD, BB_STD)
    ...
    roc_10 = (
        (price - prices[-11]) / prices[-11] * 100.0
        if len(prices) > 10 and prices[-11] != 0 else 0.0
    )
