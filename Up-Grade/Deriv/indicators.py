...
def macd(
    ...
    Implementação O(n): dois EMAs incrementais sem reconstruir fatias.
    """
...
def atr(prices: list, period: int = 14) -> Optional[float]:
    """
    Average True Range via Wilder's smoothing.

    Para dados de tick: TR[i] = max(prices[i-1], prices[i]) − min(prices[i-1], prices[i]).
    Mede a volatilidade absoluta — normalize por preço (atr / price) para comparar
    entre pares ou momentos de preços diferentes.
    Requer pelo menos period + 1 pontos.
    """
    if len(prices) < period + 1:
        return None
    ...
    tr_list: list = []
    for i in range(1, len(prices)):
        hi = max(prices[i - 1], prices[i])
        lo = min(prices[i - 1], prices[i])
        tr_list.append(hi - lo)
...
def stochastic(
    ...
    %K = (price − lowest_k) / (highest_k − lowest_k) × 100
    %D = EMA(%K_history, d_period)  ← linha de sinal

    Retorna (%K, %D) ou None se dados insuficientes.
    Sobrecomprado > 80, sobrevendido < 20.
    Divergência %K vs %D sinaliza reversão.
    """
    ...
    for i in range(k_period - 1, len(prices)):
        low_k  = min(prices[i - k_period + 1 : i + 1])
        high_k = max(prices[i - k_period + 1 : i + 1])
        rng    = high_k - low_k
        k_val  = (prices[i] - low_k) / rng * 100.0 if rng != 0 else 50.0
