...
# EMA_TREND=50 é o mais exigente
_MIN_WINDOW = max(ADX_PERIOD * 2 + 1, EMA_TREND)  # ≈50 com EMA_TREND=50

# Margem extra ao fim do CSV para evitar overfitting nos últimos ticks
_LOOKAHEAD_GUARD = 20
...
def _extract_features(window: list) -> dict | None:
    ...
    # ATR normalizado (volatilidade relativa ao preço)
    atr_v    = ind.atr(window, ATR_PERIOD)
    atr_norm = (atr_v / price) if (atr_v is not None and price != 0) else 0.0

    # Stochastic %K e %D
    stoch_res = ind.stochastic(window, STOCH_K_PERIOD, STOCH_D_PERIOD)
    stoch_k   = stoch_res[0] if stoch_res is not None else 50.0
    stoch_d   = stoch_res[1] if stoch_res is not None else 50.0

    # EMA50 — tendência de médio prazo
    ema50       = ind.ema(window, EMA_TREND)
    ema50_cross = (ema9 - ema50) / price if (ema50 is not None and price != 0) else 0.0

    # Variação da largura das Bollinger (expansão = +, contração = -)
    bb_width_change = 0.0
    if bb_ is not None and len(window) > BB_PERIOD:
        bb_prev = ind.bollinger(window[:-1], BB_PERIOD, BB_STD)
        if bb_prev is not None:
            bb_upper_p, bb_mid_p, bb_lower_p = bb_prev
            band_range_p  = bb_upper_p - bb_lower_p
            bb_width_prev = band_range_p / bb_mid_p if bb_mid_p != 0 else 0.0
            bb_width_change = bb_width - bb_width_prev

    # ROC 10 — variação percentual nos últimos 10 ticks
    roc_10 = (
        (price - window[-11]) / window[-11] * 100.0
        if len(window) > 10 and window[-11] != 0 else 0.0
    )
...
def build_dataset(
    ...
    for i in range(window_size, n_ticks - _max_lookahead - _LOOKAHEAD_GUARD):
        ...
        # ── Duração ótima calculada ANTES do target ─────────────────────────
        # O target usa a duração ótima para alinhar o que o modelo aprende
        # com o contrato que o bot realmente executa.
        # Nota: usa preços futuros — oracle label inevitável em supervised learning.
        ...
        for d in CANDIDATE_DURATIONS:
            ...
            delta    = (future_p - current) * direction
            if delta > best_abs:
                best_abs = delta
                best_d   = d
        ...
        # Target alinhado com a duração ótima (não apenas o próximo tick)
        target = 1 if prices_all[i + best_d] > current else 0
