...
class DerivBot:
    ...
    def _on_message(self, ws, message: str) -> None:
        ...
        if "error" in data:
            ...
            print(f"\n[BOT] Erro API [{err_code}]: {data['error']['message']}")
            # Resetar estado para não travar o bot em caso de erro durante operação
    ...
    def _send_proposal(self, ws, direction: str, indicators: dict) -> None:
        ...
        atr_norm = indicators.get("atr_norm", 0.0) or 0.0
        stake = self.risk_manager.get_stake(atr_norm)
    ...
    def _handle_contract_update(self, contract: dict) -> None:
        ...
        # Limpar todas as subscriptions de contrato para não acumular na sessão
        try:
            self._ws.send(json.dumps({"forget_all": "proposal_open_contract"}))
        except Exception:
            pass
