...
class RiskManager:
    ...
    def get_stake(self, atr_norm: float = 0.0) -> float:
        """Retorna o stake da próxima operação.

        USE_ATR_SIZING=True  → stake = ATR_RISK_USD / atr_atual
                                (stake menor quando mercado volátil)
        USE_ATR_SIZING=False → STAKE_PCT % do saldo atual (comportamento anterior)
        """
        if USE_ATR_SIZING and atr_norm > 0.0:
            atr_abs = atr_norm * self.balance
            stake   = round(ATR_RISK_USD / atr_abs, 2) if atr_abs > 0 else 0.0
            # Cap de 3× STAKE_PCT para evitar stakes absurdos em mercado calmo
            max_stake = round(self.balance * STAKE_PCT * 3, 2)
